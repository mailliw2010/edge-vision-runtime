...
- `SourceApp` / `SourceSession`：承接 source URI、受限 ffmpeg / GStreamer appsink 解码入口，并为后续拉流、解码、帧分发常驻化预留边界
...
- Jetson NVDEC / DeepStream 常驻解码链路，以及 TensorRT engine 构建/部署策略完善
